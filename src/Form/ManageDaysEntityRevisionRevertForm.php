...
/**
 * Provides a form for reverting a Manage days entity revision.
 *
 * @ingroup booking_manager
 */
class ManageDaysEntityRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'manage_days_entity_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Manage days entity: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Manage days entity %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\booking_manager\Entity\ManageDaysEntityInterface $revision
   ...
   */
  protected function prepareRevertedRevision(ManageDaysEntityInterface $revision, FormStateInterface $form_state) {
